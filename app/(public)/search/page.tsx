...
import { getAllBooks } from "@/lib/book-store"
...
function SearchResultsContent() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      const searchResults = searchBooks(q, books)
  ...
  if (!q.trim()) {
    return (
      <div className="flex items-center justify-center py-20">
        <div className="text-zinc-400 text-lg">Enter a search term to find books.</div>
      </div>
    )
  }
                      ...
                      {result.matches.map((match, idx) => {
                        ...
                        return (
                          <div key={idx} className="text-sm">
                            <Link
                              href={`/books/${result.bookSlug}?page=${page}`}
