...
interface PasswordModalProps {
  ...
  onUnlocked: (brochure: Brochure) => void
}
...
export function PasswordModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (hash === brochure.passwordHash) {
      sessionStorage.setItem("unlocked:" + brochure.id, "true")
      onUnlocked(brochure)
    } else {
      setLoading(false)
      setError(true)
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{brochure?.title ?? "Enter Password"}</DialogTitle>
        </DialogHeader>
