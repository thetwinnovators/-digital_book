...
import type { BookElement, TextContent, ImageContent, VideoContent, AudioContent } from "@/lib/types"
...
interface ElementWrapperProps {
  ...
  selected: boolean
  onSelect: () => void
  onUpdate: (updates: Partial<BookElement>) => void
  mediaUrls: Record<string, string>
  siblingElements: BookElement[]
}
...
export default function ElementWrapper({
  ...
  const handleMouseDownMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      // If this is a selected text element, allow default so the cursor
      // can be placed inside the contentEditable div
  ...
  if (element.type === "text") {
    ...
    contentNode = src ? (
      ...
      <div className="w-full h-full bg-zinc-700 flex items-center justify-center text-zinc-400 text-xs rounded">Video</div>
    )
  } else if (element.type === "audio") {
    const content = element.content as AudioContent
    const src = content.url || mediaUrls[content.mediaId] || null
    contentNode = src ? (
      <AudioPlayer src={src} loop={content.loop} autoPlay={content.autoplay} interactive={selected} />
    ) : (
      <div className="w-full h-full bg-zinc-700 flex items-center justify-center text-zinc-400 text-xs rounded">Audio</div>
    )
  }
